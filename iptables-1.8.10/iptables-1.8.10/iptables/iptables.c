...
static bool
rule_is_princeps(const struct ipt_entry *fw)
{
	bool princeps = false;

	IPT_MATCH_ITERATE(fw, match_is_princeps, &princeps);
	return princeps;
}
...
flush_non_princeps_rules(const xt_chainlabel chain, struct xtc_handle *handle)
{
	...
	for (fw = iptc_first_rule(chain, handle); fw != NULL;
	     fw = iptc_next_rule(fw, handle)) {
		if (!rule_is_princeps(fw))
			delete_count++;
	}
	...
	for (fw = iptc_first_rule(chain, handle), index = 0, i = 0;
	     fw != NULL;
	     fw = iptc_next_rule(fw, handle), ++index) {
		if (rule_is_princeps(fw))
			continue;
		delete_list[i++] = index;
	}
	...
	for (i = delete_count; i > 0; --i) {
		if (!iptc_delete_num_entry(chain, delete_list[i - 1], handle)) {
			free(delete_list);
			return 0;
		}
	}
...
static bool
chain_has_princeps_rules4(const xt_chainlabel chain, struct xtc_handle *handle)
{
	const struct ipt_entry *fw;
...
int
delete_chain4_preserve_princeps(const xt_chainlabel chain, int verbose,
			       struct xtc_handle *handle)
{
	if (chain_has_princeps_rules4(chain, handle))
		return 1;

	return delete_chain4(chain, verbose, handle);
}
