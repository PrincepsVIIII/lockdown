...
static struct option original_opts[] = {
	...
	{.name = "princeps-rule", .has_arg = 0, .val = XT_OPT_PRINCEPS_RULE},
	    ...
	    int numeric)
{
	...
	if (xtables_match_is_princeps(m))
		return 0;
...
static int
match_is_princeps(const struct xt_entry_match *match, bool *princeps)
{
	if (!xtables_match_is_princeps(match))
		return 0;
...
static bool
rule_is_princeps(const struct ip6t_entry *fw)
{
	bool princeps = false;

	IP6T_MATCH_ITERATE(fw, match_is_princeps, &princeps);
	return princeps;
}
...
static int
visible_insert_index6(const xt_chainlabel chain, int rulenum,
		      bool include_princeps, struct xtc_handle *handle)
{
	const struct ip6t_entry *fw;
	unsigned int actual_num, visible_num;
...
static int
flush_non_princeps_rules(const xt_chainlabel chain, struct xtc_handle *handle)
{
	const struct ip6t_entry *fw;
	unsigned int delete_count = 0, index, i;
	unsigned int *delete_list;

	for (fw = ip6tc_first_rule(chain, handle); fw != NULL;
	     fw = ip6tc_next_rule(fw, handle)) {
		if (!rule_is_princeps(fw))
			delete_count++;
	}
	...
	delete_list = xtables_malloc(sizeof(*delete_list) * delete_count);
	for (fw = ip6tc_first_rule(chain, handle), index = 0, i = 0;
	     fw != NULL;
	     fw = ip6tc_next_rule(fw, handle), ++index) {
		if (rule_is_princeps(fw))
			continue;
		delete_list[i++] = index;
	}

	for (i = delete_count; i > 0; --i) {
		if (!ip6tc_delete_num_entry(chain, delete_list[i - 1], handle)) {
			free(delete_list);
			return 0;
		}
	}
...
static bool
chain_has_princeps_rules6(const xt_chainlabel chain, struct xtc_handle *handle)
{
	const struct ip6t_entry *fw;
	      ...
	      struct xtc_handle *handle)
{
	...
	return flush_non_princeps_rules(chain, handle);
}
...
int
delete_chain6_preserve_princeps(const xt_chainlabel chain, int verbose,
			       struct xtc_handle *handle)
{
	if (chain_has_princeps_rules6(chain, handle))
		return 1;

	return delete_chain6(chain, verbose, handle);
}
		       ...
		       struct xtc_handle *h, const char *chain, int counters)
{
	...
	/* print chain name */
	printf("-A %s", chain);
	if (rule_is_princeps(e))
		printf(" --princeps-rule");
	...
	case CMD_INSERT:
		{
			...
			insert_index = visible_insert_index6(chain, rulenum,
							     cs.options & OPT_PRINCEPS_RULE,
							     *handle);
			if (insert_index < 0) {
				errno = E2BIG;
