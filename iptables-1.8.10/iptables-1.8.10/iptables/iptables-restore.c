	...
	while (fgets(buffer, sizeof(buffer), in)) {
		...
		if (buffer[0] == '\n')
			...
			if (cb->ops->builtin(chain, handle) <= 0) {
				...
				} else if (cb->ops->is_chain(chain, handle)) {
					DEBUGP("Reusing preserved user defined chain '%s'\n",
					       chain);
				} else {
...
static const struct iptables_restore_cb ipt_restore_cb = {
	...
	.delete_chain	= delete_chain4_preserve_princeps,
...
static const struct iptables_restore_cb ip6t_restore_cb = {
	...
	.delete_chain	= delete_chain6_preserve_princeps,
