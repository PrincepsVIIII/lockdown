...
static void xtables_add_princeps_rule_match(struct iptables_command_state *cs)
{
	struct xtables_match *match;
	struct xt_comment_info *info;
	size_t size;

	match = xtables_find_match("comment", XTF_LOAD_MUST_SUCCEED,
				   &cs->matches);
	...
	strcpy(match->m->u.user.name,
	       match->real_name ? match->real_name : match->name);

	xs_init_match(match);

	info = (struct xt_comment_info *)match->m->data;
	strncpy(info->comment, XTABLES_PRINCEPS_RULE_MARKER,
		XT_MAX_COMMENT_LEN - 1);
	info->comment[XT_MAX_COMMENT_LEN - 1] = '\0';
}

bool xtables_match_is_princeps(const struct xt_entry_match *match)
{
	const struct xt_comment_info *info;

	if (strcmp(match->u.user.name, "comment") != 0)
		return false;

	info = (const struct xt_comment_info *)match->data;
	return strnlen(info->comment, XT_MAX_COMMENT_LEN) ==
	       strlen(XTABLES_PRINCEPS_RULE_MARKER) &&
	       strcmp(info->comment, XTABLES_PRINCEPS_RULE_MARKER) == 0;
}

bool xtables_rule_match_is_princeps(const struct xtables_rule_match *matchp)
{
	return matchp != NULL && matchp->match != NULL &&
	       matchp->match->m != NULL &&
	       xtables_match_is_princeps(matchp->match->m);
}

void xtables_restore_princeps_rule(struct iptables_command_state *cs)
{
	struct xtables_rule_match *matchp;

	cs->princeps_rule = false;
	for (matchp = cs->matches; matchp; matchp = matchp->next) {
		if (!xtables_rule_match_is_princeps(matchp))
			continue;
		cs->princeps_rule = true;
		break;
	}
}

void xtables_ensure_princeps_rule(struct iptables_command_state *cs)
{
	struct xtables_rule_match *matchp;
	...
	if (cs->princeps_rule && !has_marker)
		xtables_add_princeps_rule_match(cs);
}
...
int print_match_save(const struct xt_entry_match *e, const void *ip)
{
	...
	if (xtables_match_is_princeps(e))
		return 0;
	      ...
	      struct xtables_args *args)
{
	...
	while ((cs->c = getopt_long(argc, argv,
		...
		case XT_OPT_PRINCEPS_RULE:
			if (invert)
				xtables_error(PARAMETER_PROBLEM,
					      "unexpected ! flag before --princeps-rule");
			if (cs->options & OPT_PRINCEPS_RULE)
				xtables_error(PARAMETER_PROBLEM,
					      "multiple --princeps-rule flags not allowed");
