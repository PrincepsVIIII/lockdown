...
static int nft_chain_flush_non_princeps(struct nft_handle *h, struct nft_chain *c,
				       bool verbose);
		 ...
		 const char *chain, bool verbose, bool skip)
{
	...
	if (chain) {
		...
		if (!c)
			return;
		nft_chain_flush_non_princeps(h, c, verbose);
		return;
	}

	nft_cache_sort_chains(h, table);
	nft_chain_foreach(h, table, nft_rule_flush_cb, &d);
}

static bool nft_rule_is_princeps(struct nft_handle *h, struct nftnl_rule *r)
{
	struct iptables_command_state cs = {};
	bool princeps = false;

	if (h->ops->rule_to_cs(h, r, &cs))
		princeps = cs.princeps_rule;
	if (h->ops->clear_cs)
		h->ops->clear_cs(&cs);
...
{
	struct nftnl_rule_iter *iter;
	struct nftnl_rule **delete_list = NULL;
	struct nftnl_rule *r;
	unsigned int delete_count = 0, i = 0;
	const char *chain;

	chain = nftnl_chain_get_str(c->nftnl, NFTNL_CHAIN_NAME);
	if (verbose)
	...
	for (i = 0; i < delete_count; ++i) {
		if (__nft_rule_del(h, delete_list[i]) < 0) {
			free(delete_list);
			return -1;
		}
	}
...
static int nft_rule_flush_cb(struct nft_chain *c, void *data)
{
	...
	return nft_chain_flush_non_princeps(d->h, c, d->verbose);
}
		   ...
		   bool verbose)
{
	...
	if (chain) {
		...
		return nft_chain_flush_non_princeps(h, c, verbose) == 0;
	}
...
static int __nft_table_flush(struct nft_handle *h, const char *table, bool exists)
{
	...
	nft_cache_sort_chains(h, table);
	return nft_chain_foreach(h, table, nft_rule_flush_cb, &d);
}
