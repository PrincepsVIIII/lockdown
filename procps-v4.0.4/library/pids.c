...
static inline unsigned pids_princeps_flags (
        unsigned flags)
{
    if (procps_princeps_filter_active())
        flags |= f_stat;
    return flags;
} // end: pids_princeps_flags
        ...
        struct pids_info *info)
{
 ...
 #define n_saved  info->fetch.n_alloc_save
    ...
    while (info->read_something(info->fetch_PT, &info->fetch_proc)) {
        if (procps_princeps_hidden(&info->fetch_proc))
            continue;
        if (!(n_inuse < n_alloc)) {
            n_alloc += STACKS_GROW;
...
fresh_start:
        if (!pids_oldproc_open(&info->get_PT, pids_princeps_flags(info->oldflags)))
    ...
    || (info->get_PT && info->get_PT->flags != pids_princeps_flags(info->oldflags))) {
        pids_oldproc_close(&info->get_PT);
        goto fresh_start;
    }
    errno = 0;
    ...
    do {
        if (NULL == info->read_something(info->get_PT, &info->get_proc))
            return NULL;
    } while (procps_princeps_hidden(&info->get_proc));
        ...
        enum pids_fetch_type which)
{
    ...
    if (!pids_oldproc_open(&info->fetch_PT, pids_princeps_flags(info->oldflags)))
        return NULL;
    info->read_something = which ? readeither : readproc;
        ...
        enum pids_select_type which)
{
    ...
    if (!pids_oldproc_open(&info->fetch_PT,
            (pids_princeps_flags(info->oldflags) | which), ids, numthese))
        return NULL;
