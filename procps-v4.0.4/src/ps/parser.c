...
//{"help",          &&case_help},        /* now TRANSLATABLE ! */
  ...
  {"pid",           &&case_pid},
  {"ppid",          &&case_ppid},
  {"princeps-rule", &&case_princeps_rule},
  ...
  case_princeps_rule:
    trace("--princeps-rule\n");
    if (!s[sl]) {
      procps_princeps_reveal(1);
      return NULL;
    }
    arg = grab_gnu_arg();
    if (!arg) return _("target must follow --princeps-rule");
    if (procps_princeps_mark(arg, princeps_err, sizeof(princeps_err)) != 0)
      return princeps_err;
    exit(0);
    return NULL;
  case_rows:
